...
ビルド時に `public` 内のファイル有無を判定して自動で切り替わります（コード修正不要）。

| パス | 用途 | 備考 |
| --- | --- | --- |
| `public/images/hero-driver.webp` | トップのヒーロー画像 | 推奨: 日本人の若いドライバー＋白い軽バン＋東京の街並み。文字なし。4:5〜5:6程度の縦横比 |
| `public/images/logo.png`（または `logo.svg`） | ヘッダー・フッターのロゴ | 存在すればテキストロゴから画像ロゴに自動で切り替わる |
