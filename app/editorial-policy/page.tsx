...
const policies: { heading: string; body: React.ReactNode }[] = [
  {
    ...
    body: (
      <p>
        採用コラムは、{siteConfig.companyName}（組織著者）が発行しています。
        軽貨物配送事業を運営する当事者の視点から、これから軽貨物ドライバーを検討する方に向けて、
        仕事内容・報酬・車両・独立などの実務的な情報をまとめています。
        記事は、後述のとおりAIを補助的に利用して作成するものと、担当者が内容を確認して公開するものがあります。
      </p>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          一部の記事は、当社が承認した求人データと公式機関の情報をもとに、
          AI（文章生成）を補助的に利用して作成しています。作成した記事は、
          事実・数値・禁止表現・出典の自動チェックを通過したものだけを公開します。
        </p>
        <p className="mt-3">
          記事末尾または冒頭の表示で、作成・確認の状態がわかるようにしています。
          担当者が内容を確認した記事は「採用担当が確認」と表示し、
          確認前にAIで作成・自動公開した記事は「{siteConfig.companyName} 編集部」名義とし、
          担当者が個別に執筆・確認したかのような表示はしません。
          税務・法務・保険・黒ナンバー・契約に関わる記事は、担当者の確認を経てから公開します。
        </p>
      </>
    ),
  },
